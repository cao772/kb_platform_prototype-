...
class Handler(BaseHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/collection-experience/access-preconditions":
            try:
                self._send_json(collection_experience.access_preconditions())
            except Exception as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
            return
        ...
        if parsed.path == "/api/collection-experience/transferability":
            try:
                self._send_json(collection_experience.transferability())
            except Exception as exc:
                self._send_json({"error": str(exc)}, HTTPStatus.BAD_REQUEST)
            return
